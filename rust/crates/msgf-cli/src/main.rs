...
//!
//! One subcommand today: `rescore`. Given a spectra file (MGF) and a list of peptide-spectrum
//! matches, it recomputes MS-GF+'s **RawScore**, **DeNovoScore**, and **SpecEValue** (the spectral
//! E-value / p-value) for each PSM. This is the per-spectrum significance scoring the library
//! implements — it is *not* a database search: candidate peptides come from the input PSM list,
//! there is no fragment-index candidate generation (that is the future `msgf-search` engine).
//!
...
fn main() -> ExitCode {
    ...
    match args.first().map(String::as_str) {
        Some("rescore") => match Config::parse(&args[1..]) {
            Ok(cfg) => match run_rescore(&cfg) {
                Ok(()) => ExitCode::SUCCESS,
                Err(e) => {
                    eprintln!("msgf rescore: {e}");
                    ExitCode::FAILURE
                }
            },
            Err(e) => {
...
fn run_rescore(cfg: &Config) -> Result<(), String> {
...
///
/// - **C-terminal (peptide) cleavage:** credit if the last residue is K or R, or the peptide sits
///   at the protein C-terminus (flanking `-`); otherwise penalty.
...
fn cleavage_score(pep: &str, residues: &[msgf_chem::peptide::Residue]) -> i32 {
    ...
    let cterm = if is_kr(last) || flank_c == Some(b'-') {
        CREDIT
    } else {
        PENALTY
    };
