...
1 % FDR" can be compared to Java at all. What we *do* already have is an oracle —
`validation/golden/iprg2013_F13.golden.json` (regenerated locally, never committed) carries
`protein`, `qvalue` and `pep_qvalue` per PSM,
...
*Gate 1 (the search golden):* reproduce `qvalue` and `pep_qvalue` from `iprg2013_F13.golden.json`
exactly. That golden is MS-GF+-derived, so it is regenerated locally and the test skips without it.
Requires rolling the `-unroll 1` TSV rows
...
lookup rule. Frozen to `validation/golden/fdr/fdrmap_cases.golden.json` — MS-GF+-derived, so
gitignored and regenerated locally, with its sha256 in `golden/UC_DERIVED.sha256` — and checked
...
Measured from the golden: 4133 unrolled rows → **1610 unique PSMs**, of which **765 are
...
- **Done:** `validation/golden/fdr/fdrmap_cases.golden.json` (Java probe, JVM + jar only, and
  gitignored like every MS-GF+-derived golden — see `LICENSING.md`), generated by
  `validation/reference/make_fdr_golden.sh` (also run by
