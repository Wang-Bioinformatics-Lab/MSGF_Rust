...
1 % FDR" can be compared to Java at all. What we *do* already have is an oracle — the
`validation/golden/iprg2013_F13.golden.json` (MS-GF+-derived, so generated locally rather than
committed) carries `protein`, `qvalue` and `pep_qvalue` per PSM,
...
The golden is MS-GF+-derived, so it is generated locally (`reference/generate_golden.sh`) and the
test skips without it. Requires rolling the `-unroll 1` TSV rows
...
gitignored and regenerated by `make_fdr_golden.sh` — and checked
