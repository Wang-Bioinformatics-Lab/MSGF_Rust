...
lookup rule. Frozen to `validation/golden/fdr/fdrmap_cases.golden.json` — MS-GF+-derived, so gitignored and
regenerated locally — and checked bit-for-bit by `msgf-fdr/tests/golden_fdrmap.rs` — 110 thresholds, 556 lookups — plus a no-JVM
