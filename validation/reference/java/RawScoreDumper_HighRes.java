...
/**
 * RawScoreDumper_HighRes -- HighRes twin of RawScoreDumper.
 *
 * IDENTICAL to RawScoreDumper.java in every respect except the MODEL_NAME label
 * constant, which is set to "HCD_HighRes_Tryp.param" so the emitted JSON "model"
 * field honestly names the HighRes model the F13 search actually used (-inst 1 =
 * HighRes). The actual scoring model is loaded from args[0] in both classes;
 * this copy exists only to keep the emitted label correct.
 *
 * Validation oracle for the FINAL RawScore stage of the Rust MS-GF+ port. For
 * each selected (scan, charge, golden_peptide) it reconstructs MS-GF+'s peptide
 * prefix-mass arrays EXACTLY as edu.ucsd.msjava.msdbsearch's CandidatePeptideGrid
 * + DBScanner do, then runs the reference FastScorer (node summation, no edges)
 * and DBScanScorer (node + edge) getScore(...) and dumps node_only / full / edge
 * scores. Every number comes from actually running MS-GF+ (MSGFPlus.jar). No
 * value is fabricated.
 *
 ...
 *
 * The scored spectrum is built EXACTLY as ScoredSpectrumDumper_HighRes builds it
 ...
 * f13_scored_spectrum_highres.golden.json prefix/suffix arrays the Rust port
 * validates against.
 *
 ...
 */
public class RawScoreDumper_HighRes {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length != 5) {
            System.err.println("usage: RawScoreDumper_HighRes <model.param> <spectra.mgf> <modfile> <selection.tsv> <out.json>");
            System.exit(2);
        }
        ...
        // 1. Scoring model, built exactly as ScoredSpectrumDumper_HighRes / the
        //    golden scored-spectrum pipeline (readFromFile, no doNotUseError()).
        ...
        ParamManager pm = new ParamManager("RawScoreDumper_HighRes", "1", "2026", "usage");
        ...
        sb.append(" \"note\": ").append(jstr(
            ...
            + "RawScoreDumper_HighRes.java against MSGFPlus.jar; every number is from MS-GF+.")).append(",\n");
